...
# FormDB HTTP Real Data Persistence Test Script
...
echo ""
echo "=== FormDB HTTP Real Data Persistence Test ==="
echo ""
...
DB_RESPONSE=$(curl -s -X POST "$BASE_URL/databases" \
  ...
  -d '{
    "path": "/tmp/formdb_persistence_test.db"
  }')
echo "$DB_RESPONSE" | jq '.'
...
echo ""
echo "Note: M12 uses CBOR encoding and real FormDB NIF operations."
echo "Data is persisted to /tmp/formdb_persistence_test.db"
echo ""
